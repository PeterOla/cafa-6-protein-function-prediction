#!/usr/bin/env python3
"""
Quick GBDT alignment verification using Cell 13a data loading approach.
Uses X_train_mmap.npy (full 7168 dims) + Y built from train_seq.feather ordering.

Expected F1: ~0.15-0.30 if aligned, ~0.003 if broken.
"""
import sys
import time
import numpy as np
...
N_TERMS = 200          # Subset of terms (GBDT is slower)
TEST_SIZE = 0.2

print("=" * 60)
print("QUICK GBDT ALIGNMENT VERIFICATION")
print("=" * 60)

# === Check for py_boost ===
try:
    from py_boost import GradientBoosting
    print("✓ py_boost available")
    USE_PYBOOST = True
except ImportError:
    print("⚠ py_boost not found, falling back to sklearn GradientBoosting")
    USE_PYBOOST = False
...
# === 7. Train GBDT ===
print(f"\n[7] Training GBDT on {N_TERMS} terms...")
t0 = time.time()

val_preds = np.zeros_like(Y_val, dtype=np.float32)

if USE_PYBOOST:
    # py_boost can do multi-output
    print("  Using py_boost (GPU if available)...")
    try:
        ...
        print(f"  py_boost failed: {e}")
        print("  Falling back to sklearn...")
        USE_PYBOOST = False

if not USE_PYBOOST:
    # sklearn per-term (slower)
    print("  Using sklearn GradientBoosting (per-term)...")
    for i in range(N_TERMS):
        if (i + 1) % 50 == 0 or i == 0:
            print(f"  Term {i+1}/{N_TERMS}...")
...
# === 8. Evaluate F1 ===
print("\n[8] Evaluating F1 at various thresholds...")
