#!/usr/bin/env python3
"""
Quick LogReg alignment verification using Cell 13a data loading approach.
...
N_TERMS = 500          # Subset of terms (top frequency)
TEST_SIZE = 0.2

print("=" * 60)
print("QUICK LOGREG ALIGNMENT VERIFICATION")
print("=" * 60)
...
# === 8. Train LogReg (one-vs-rest) ===
print(f"\n[8] Training LogReg on {N_TERMS} terms...")
t0 = time.time()

# Train separate classifier per term
...
for i in range(N_TERMS):
    ...
    y_col = Y_tr[:, i]
    if y_col.sum() < 2:  # Skip terms with <2 positives
        continue
    ...
    clf.fit(X_tr_scaled, y_col)
    val_preds[:, i] = clf.predict_proba(X_val_scaled)[:, 1]

train_time = time.time() - t0
print(f"  Training complete: {train_time:.1f}s")

# === 9. Evaluate F1 ===
print("\n[9] Evaluating F1 at various thresholds...")
